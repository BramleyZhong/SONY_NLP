...
# Evaluate the BART model with Beam Search
def beam_search_decode(input_ids, max_length, num_beams):
    # Generate sequences using beam search
    beam_output = model.generate(input_ids, max_length=max_length, num_beams=num_beams, early_stopping=True)
    return beam_output

model.eval()
val_output = beam_search_decode(val_input_ids, max_length=16, num_beams=4)  # You can adjust num_beams
...
# Compute BLEU score
references = [[true_sku] for true_sku in val_sku_names]
hypotheses = val_output_text
bleu_score = corpus_bleu(references, hypotheses)
print(f"BLEU Score: {bleu_score * 100:.2f}%")

# Export DataFrame to Excel
output_file = 'C:/Users/Lenovo/Desktop/Web Scraping/matched_tv_models_beam_BLEU_1.xlsx'
